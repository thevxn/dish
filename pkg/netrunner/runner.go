...
func RunSocketTest(sock socket.Socket, out chan<- socket.Result, wg *sync.WaitGroup, timeoutSeconds uint, logger *log.Logger) {
	defer wg.Done()
	defer close(out)
...
func NewNetRunner(sock socket.Socket, logger *log.Logger) (NetRunner, error) {
	...
	if exp.MatchString(sock.Host) {
		return httpRunner{&http.Client{}, logger}, nil
	}

	return tcpRunner{logger}, nil
}
...
func (runner tcpRunner) RunTest(ctx context.Context, sock socket.Socket) socket.Result {
	...
	runner.logger.Println("TCP runner: connect: " + endpoint)
...
func (runner httpRunner) RunTest(ctx context.Context, sock socket.Socket) socket.Result {
	...
	runner.logger.Println("HTTP runner: connect:", url)
